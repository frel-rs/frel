...
A reactive, truly platform-independent UI library for Rust.

By "truly" I mean that the library is not tied to HTML or any other presentation
layer in any way. It has a declarative DSL and uses adapters for platform-specific
rendering and input.
...
In this case check [overview.md](doc/00_overview/10_overview.md).
...
If you would like to use AI for summarizing the documentation, You can run the included `utils`
crate, which compiles the documentation into a single Markdown file (`./target/compiled.md`).
